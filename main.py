...
stopuser = {}
token = '6508582139:AAGyiFDRn8SD7bEAg7uNOZ86w-CrEJLkEKg'
...
def start(message):
	def my_function():
		...
		try:
			...
			pas ='CHEETAH-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))
			current_time = datetime.now()
